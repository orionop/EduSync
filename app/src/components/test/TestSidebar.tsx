...
const TestSidebar: React.FC<TestSidebarProps> = ({ isOpen, onClose }) => {
  const location = useLocation();
  const { signOut, user } = useAuth();

  const menuItems = [
    { path: '/test/dashboard', icon: LayoutDashboard, label: 'Dashboard' },
    { path: '/test/exam-prerequisites', icon: FileCheck, label: 'Exam Prerequisites' },
    { path: '/test/exam-timetable', icon: Calendar, label: 'Exam Timetable' },
    { path: '/test/results', icon: Award, label: 'Results' },
    { path: '/test/kt-section', icon: RefreshCw, label: 'KT Section' },
    { path: '/test/submissions', icon: Upload, label: 'Submissions' },
    { path: '/test/placement-eligibility', icon: GraduationCap, label: 'Placement Eligibility' },
  ];
        ...
        <div className="p-3 border-t border-slate-200 flex-shrink-0">
          <button
            ...
            className="
              ...
              transition-all duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:ring-offset-2
            "
          >
            <LogOut className="w-[18px] h-[18px] text-slate-400" aria-hidden="true" />
            <span>Sign Out</span>
          </button>
        </div>
      </aside>
