...
const FacultySidebar: React.FC<FacultySidebarProps> = ({ isOpen, onClose }) => {
  ...
  const menuItems = [
    ...
    { path: '/faculty/marks-calculation', icon: Calculator, label: 'Marks Calculation' },
    { path: '/faculty/view-results', icon: FileText, label: 'View Results' },
    { path: '/faculty/end-semester-evaluation', icon: ClipboardCheck, label: 'End Semester Evaluation' },
    { path: '/faculty/revaluation', icon: RefreshCw, label: 'Revaluation' },
    { path: '/faculty/proctor-section', icon: Shield, label: 'Proctor Section' },
    { path: '/faculty/supervisor-duty', icon: Calendar, label: 'Supervisor Duty' },
  ];
        ...
        <div className="h-14 px-4 flex items-center justify-between border-b border-slate-200 flex-shrink-0">
          <div className="flex items-center">
            ...
            <div>
              ...
              <span className="block text-[11px] text-slate-500 -mt-0.5">Faculty Portal</span>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="px-4 py-4 border-b border-slate-100 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-500 truncate">{user?.department || 'Computer Science'}</p>
            </div>
          </div>
        </div>
