...
import { Bell, PanelLeftClose, PanelLeft, LayoutDashboard, Calculator, FileText, Shield, Calendar, ClipboardCheck, MoreHorizontal } from 'lucide-react';
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <FacultyBottomNav onMoreClick={() => setSidebarOpen(true)} />
      </main>
...
const FacultyBottomNav: React.FC<{ onMoreClick: () => void }> = ({ onMoreClick }) => {
  const location = useLocation();

  const navItems = [
    { path: '/faculty/dashboard', icon: LayoutDashboard, label: 'Home' },
    { path: '/faculty/marks-calculation', icon: Calculator, label: 'Marks' },
    { path: '/faculty/view-results', icon: FileText, label: 'Results' },
    { path: '/faculty/proctor-section', icon: Shield, label: 'Proctor' },
  ];
