...
import { generateApplicationSummary } from '../../../utils/pdfGenerator';
  ...
  const handleDownloadPDF = (application: Application) => {
    try {
      generateApplicationSummary({
        ...
        studentId: application.rollNumber,
        course: application.course,
        semester: application.semester.toString(),
        applicationType: 'Examination Application',
        submissionDate: application.appliedDate,
        status: 'Pending',
        subjects: application.subjects.map((subject, index) => ({
          code: `SUB${(index + 1).toString().padStart(3, '0')}`,
          name: subject,
        })),
          ...
          <button
            onClick={() => {
              try {
                ...
                doc.text('Applications Report', 105, 25, { align: 'center' });
                doc.setFontSize(10);
                doc.text(`Generated: ${new Date().toLocaleDateString('en-IN')}`, 105, 32, { align: 'center' });

                autoTable(doc, {
                  startY: 42,
                  head: [['#', 'Student Name', 'Roll No', 'Course', 'Semester', 'Applied Date', 'Subjects']],
                  body: applications.map((app, index) => [
                    (index + 1).toString(),
                    app.studentName,
                    app.rollNumber,
                    app.course,
                    app.semester.toString(),
                    new Date(app.appliedDate).toLocaleDateString('en-IN'),
                    app.subjects.length.toString(),
                  ]),
