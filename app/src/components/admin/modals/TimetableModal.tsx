          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className={`w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white transition-colors ${
                  errors.examTitle 
                    ? 'border-red-300 dark:border-red-600 focus:border-red-500' 
                    : 'border-gray-300 dark:border-gray-600'
                }`}
                placeholder="Enter exam title"
                aria-invalid={errors.examTitle ? 'true' : 'false'}
                aria-describedby={errors.examTitle ? 'examTitle-error' : undefined}
                required
              />
              {errors.examTitle && (
                <p id="examTitle-error" className="mt-1 text-sm text-red-600 dark:text-red-400 flex items-center gap-1" role="alert">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                {errors.branch && (
                  <p id="branch-error" className="mt-1 text-sm text-red-600 dark:text-red-400 flex items-center gap-1" role="alert">
                    <AlertCircle className="h-4 w-4" aria-hidden="true" />
                    {errors.branch}
                  </p>
                )}
            ...
            <button
              ...
              onClick={() => {
                try {
                  ...
                  doc.text(formData.examTitle || 'Examination Timetable', 105, 25, { align: 'center' });
                  
                  doc.setFontSize(11);
                  doc.text(`Branch: ${formData.branch || 'All'} | Semester: ${formData.semester || 'All'}`, 105, 33, { align: 'center' });

                  // Table from generated schedule
                  const examDays = generatedSchedule.filter(s => s.type === 'exam');
                  
                  autoTable(doc, {
                    startY: 45,
                    head: [['#', 'Date', 'Subject', 'Time']],
                    body: examDays.map((schedule, index) => [
                      (index + 1).toString(),
                      new Date(schedule.date).toLocaleDateString('en-IN', { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric' }),
                      schedule.subject || '-',
                      schedule.timeSlot || formData.timeSlot,
                    ]),
                  ...
                  doc.save(`Timetable_${formData.branch || 'All'}_Sem${formData.semester || 'All'}.pdf`);
