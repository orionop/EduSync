...
const HallTickets: React.FC = () => {
  ...
  const handleDownloadHallTicket = (student: Student) => {
    try {
      const studentInfo = {
        name: student.name,
        studentId: student.rollNumber,
        email: `${student.rollNumber.toLowerCase()}@edvantage.edu.in`,
      ...
      // Mock exam schedule
      const examSchedule = [
        { subjectCode: 'SUB001', subjectName: 'Subject 1', date: '2025-03-20', startTime: '10:00 AM', endTime: '01:00 PM', venue: 'Hall A', type: 'Theory' as const },
        { subjectCode: 'SUB002', subjectName: 'Subject 2', date: '2025-03-22', startTime: '10:00 AM', endTime: '01:00 PM', venue: 'Hall B', type: 'Theory' as const },
      ];

      generateHallTicket(studentInfo, examSchedule);
      toast.success(`Hall ticket generated for ${student.name}`);
    } catch (error) {
  ...
  const handleGenerateHallTickets = () => {
    ...
    selectedStudents.forEach(studentId => {
      ...
      if (student) {
        handleDownloadHallTicket(student);
      }
    });
  };
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => {
              ...
              filteredStudents.forEach(student => handleDownloadHallTicket(student));
              toast.success(`Generating ${filteredStudents.length} hall tickets...`);
            }}
              ...
              <tr key={student.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <button
                    ...
                    onClick={() => handleDownloadHallTicket(student)}
